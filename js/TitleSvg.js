...
// 编舞:等生命体加载动画退场后再接棒;
//      若生命体未出场(v15 六路预判命中),立即播放,不作等待。
// 场次:进站首演一场,站内换页不返场,刷新即重开。
//      判据不押注 pjax,直接问浏览器"来路"(Navigation Timing):
//        来路 = reload → 无条件重演(刷新重置);
//        其余来路     → 查 sessionStorage 场记,演过即静置(换页不返场);
//        场记不存在   → 新进站,首演一次并落记。
//      另以 documentElement.dataset 作本文档内第二道防线:
//      若站点启用 pjax(换页不换文档),同一文档内也绝不二次开演。

(function () {
    // 配置
    const STROKE_DURATION = 3000; // 描边时长:2300 → 3000,笔速放缓(经 --stroke-duration 下发给 CSS)
    const HANDOFF_DELAY = 450;    // 生命体完全落幕后多留半拍白,再动笔(300 → 450)
    const HANDOFF_TIMEOUT = 2000; // 活性轮询间隔:事件丢失时最迟 2s 内自愈
    const SESSION_KEY = 'titleAnimationPlayed'; // 跨页场记:同一标签页内存续,新开标签即新场
    const PLAYED_FLAG = 'titleSvgPlayed';       // 本文档场记:挂于 documentElement.dataset

    // 防重复装订:脚本若被重复执行,只认第一次的监听
    if (window.__titleSvgBound) return;
    window.__titleSvgBound = true;

    // 问明来路:reload / navigate / back_forward / prerender
    ...
    // 跨页场记(隐私模式等存储不可用时,当作未演 —— 宁可多演,不可哑场)
    let visited = null;
    try { visited = sessionStorage.getItem(SESSION_KEY); } catch (e) {}

    function playedThisDocument() {
        return document.documentElement.dataset[PLAYED_FLAG] === 'true';
    }

    // 静置判定:本文档已演过(pjax 防线),或带着场记的站内换页(非刷新)
    function shouldStayStill() {
        return playedThisDocument() || (!isReload && !!visited);
    }

    // 挂载(幂等):SVG 尚在则空转;被 pjax 换走则补挂
    function mount() {
        ...
        // 设置路径长度与笔速(CSS 侧以 var() 读取,时长的单一来源在此)
        path.style.setProperty('--path-length', path.getTotalLength());
        path.style.setProperty('--stroke-duration', STROKE_DURATION + 'ms');

        // 静置条件命中(站内换页归来):直接定格完成态,不动笔
        if (shouldStayStill()) {
            path.classList.add('no-animation');
            return;
        }

        waitForLoader(function () { play(path); });
    }

    // 交棒决策:等生命体完全落幕(life-loader:gone)再动笔。
    // 双重依据,缺一不可:
    //   dataset.lifeLoaderStatus 旗 —— 生命体开机后的状态;
    //   #life-loader 元素在场与否 —— 覆盖"预渲染/后台尚未开机"的盲区
    //     (元素在而旗未立 = 对方还没开机,必须订阅等待,绝不能误判为"无加载器")
    // 兜底为活性轮询而非墙钟:闹钟响时元素仍在 → 对方正常演出,续订;
    //   元素已不在而事件缺席 → 判定事件丢失,补播。永不误播,最迟 2s 自愈。
    function waitForLoader(then) {
        const status = document.documentElement.dataset.lifeLoaderStatus;
        const loaderPresent = !!document.getElementById('life-loader');
        if (status === 'gone' || (!status && !loaderPresent)) { then(); return; }

        let fired = false;
        function handoff() {
            if (fired) return;
            fired = true;
            document.removeEventListener('life-loader:gone', handoff);
            setTimeout(then, HANDOFF_DELAY);
        }
        document.addEventListener('life-loader:gone', handoff, { once: true });
        (function livenessPoll() {
            setTimeout(function () {
                if (fired) return;
                if (!document.getElementById('life-loader')) { handoff(); }
                else { livenessPoll(); }
            }, HANDOFF_TIMEOUT);
        })();
    }
    ...
    function play(path) {
        // 开笔即记账:两处场记同时落笔,中途换页(整页或 pjax)皆不致返场
        try { sessionStorage.setItem(SESSION_KEY, 'true'); } catch (e) {}
        document.documentElement.dataset[PLAYED_FLAG] = 'true';
    ...
    // 装订
    // 整页加载:DOM 就绪即挂载;脚本晚于 DOMContentLoaded 注入时,直接补挂
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', mount);
    } else {
        mount();
    }
    // 若站点启用 pjax:navbar 被换走时以完成态补挂;未启用则此监听终生沉默,无副作用
