...
// 功能：描边绘制 + 填充淡入 + 悬停发光 + 深色模式 + 无障碍支持
// 优化：CSS 加载监听、页面可见性检测、动画状态管理、内存泄漏防护
...
(function() {
    ...
    /**
     * 插入 SVG 到 DOM
     * @param {Element} navbarBrand 
     * @returns {Element|null} path 元素
     */
    function insertSVG(navbarBrand) {
        navbarBrand.innerHTML = `
            <svg class="title-svg" viewBox="0 0 224.496 43.824" xmlns="http://www.w3.org/2000/svg" aria-label="我不想当ai" role="img">
                <path class="title-path" d="${SVG_PATH}"/>
            </svg>
        `;
        return navbarBrand.querySelector('.title-path');
    }
    ...
    function playAnimation(path) {
        ...
        const handleVisibilityChange = () => {
            if (document.hidden && animationInProgress) {
                finishAnimation();
            }
        };
        ...
        // 动画完成
        setTimeout(finishAnimation, CONFIG.STROKE_DURATION);
    }
    ...
    async function init() {
        ...
        if (navbarBrand.querySelector('.title-svg')) return;  // 防止重复初始化
        
        const hasPlayed = sessionStorage.getItem(CONFIG.SESSION_KEY);
        
        // 先插入 SVG
        const path = insertSVG(navbarBrand);
        if (!path) return;
        ...
        // 等待样式生效
        await waitForStyles(navbarBrand);
